...
export async function GET(
  ...
  try {
    ...
    // 2. Resolve current logged-in userId from Cookies for the follow check
    ...
    // 3. Resolve user - Searching by username or ID
    ...
    const { data: user, error: userError } = await supabaseAdmin
      ...
      .maybeSingle(); // Better than .single() to avoid throwing errors
    ...
    // 4. Execute counts and follow check using the admin client
